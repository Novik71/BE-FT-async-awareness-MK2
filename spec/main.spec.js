...
describe('fetchSingleHero', function() {
  it('should be a function', function() {
    expect(fetchSingleHero).to.be.a('function');
  });
  it('should invoke the callback with no error', function(done) {
    fetchSingleHero(function(err) {
      expect(err).to.be.null;
      done();
    });
  });
  it('should invoke the callback the heroes name as the second argument', function(done) {
    fetchSingleHero(function(err, hero) {
      expect(hero).to.equal('BATMAN');
      done();
    });
  });
});

describe('fetchSingleOpponent', function() {
  it('should be a function', function() {
    expect(fetchSingleOpponent).to.be.a('function');
  });
  it('should invoke the callback with no error', function(done) {
    fetchSingleOpponent(function(err) {
      expect(err).to.be.null;
      done();
    });
  }).timeout(4000);
  it('should invoke the callback the heroes name as the second argument', function(done) {
    fetchSingleOpponent(function(err, villain) {
...
describe.only('fetchFileWithSingleCall', function() {
  it('exists', function() {
